...
def enviar_notificacao(dados_do_status):
    ...
    Args:
        dados_do_status (dict): Um dicionário contendo os detalhes do status do pedido.

    Returns:
        str: Uma string no formato JSON indicando o resultado da tentativa de envio.
    """
    # --- Extrair e Validar os Dados de Entrada ---
    
    # Extrai as informações essenciais do dicionário de entrada.
    # O método .get('chave') é uma forma segura de acessar um valor, pois retorna None
    ...
    # A função all() retorna True apenas se todos os itens na lista (status, email, id) forem verdadeiros (não nulos/vazios).
    if not all([status, email_cliente, pedido_id]):
        ...
        resultado_operacao = {
            ...
            "pedido_id": pedido_id, # Mesmo que nulo, é bom retorná-lo para depuração.
    ...
    # A estrutura condicional if/elif/else decide qual ação tomar com base no status do pedido.
    if status == 'SUCESSO':
